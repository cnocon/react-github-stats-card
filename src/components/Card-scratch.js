...
const getData = async (username, accessToken, theme = false) => {
  const allData = Promise.all([
    ...
    return [user, repos, languagesObj];
  }).then(payload => {
    return payload;
  }).catch(error => {
    console.error(error);
  ...
  const user = await allData[0];
  const repos = await allData[1];
  const languagesObj = await allData[2];
  
  const sortedLanguages = await Object.entries(languagesObj).sort((a, b) => {
    return a[1][1] - b[1][1];
  });
  ...
  let element = `<div id="${theme ? 'github-stats-card' : ''}">${header}${content}${footer}</div>`;
  ...
  //   return new Promise((resolve, reject) => {
  //     const d = getData(username, accessToken, theme);
  //     resolve(d);
  //   }).then(payload => {
  //     setUser(payload[0]);
  //     setRepos(payload[1]);
...
function Card({...props}) {
  const [userData, setUserData] = useState(async () => (await getData(props.username, props.accessToken, props.theme)[0]))
  const {username, accessToken, theme} = props;
  const allData = getData(username, accessToken, theme);
  const user = allData[0];
  

  return (
    <div id={theme ? 'github-stats-card' : ''}>
      <header><h3><a href={user.html_url} htmlTarget="_blank" rel="noopener nofollow">@{username}</a>&nbsp;on GitHub<img src="https://github.githubassets.com/images/icons/emoji/octocat.png?v8" alt="Octocat"/></h3>{user.bio ? user.bio : ``}<h5><span><b>{user.public_repos}</b>Public Repos</span><span>|</span><span><b>{user.public_gists}</b>Public Gists</span></h5></header>
    </div>
  )
};
